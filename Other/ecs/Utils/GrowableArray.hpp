...
namespace ecs
{
	...
	// Wraps an array unique_ptr instance and provides functions to retrieve items and grow the buffer size.
	template<typename T>
	class GrowableArray
	{
		private:
		...
		public:
			...
			void grow(std::size_t mCapacityOld, std::size_t mCapacityNew)
			{
				...
				growImpl(newData.get(), mCapacityOld, std::is_trivially_copyable<T>{});
				data = std::move(newData);
			}

			auto& operator[](std::size_t mI) noexcept
			{
				return data[mI];
			}
