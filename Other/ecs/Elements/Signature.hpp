...
namespace ecs
{
	namespace Impl
	{
		...
		{	
			...
			static constexpr auto hasForbiddenComponents = MPL::size<ForbiddenComponents>() > 0;
			static constexpr auto hasForbiddenTags = MPL::size<ForbiddenTags>() > 0;
			static constexpr auto hasForbiddenElements = hasForbiddenComponents || hasForbiddenTags;

			// Bitset storage for this signature.
			// One bitset is enough if there are no forbidden elements.
			template<typename TBitset>
			using BitsetStorage = std::conditional_t
			<
				!hasForbiddenElements,
				BitsetStorageSingle<TBitset>,
				BitsetStorageDouble<TBitset>
			>;
		};
	}
}