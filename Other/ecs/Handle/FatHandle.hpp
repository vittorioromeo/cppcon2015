...
namespace ecs
{
    namespace Impl
    {
        ...
        public:
            FatHandle(
                TManager& mManager, const SlimHandle& mSlimHandle) noexcept
                : slimHandle{mSlimHandle},
                  manager{mManager}
            {
            }
            ...
            {
                return manager.template matchesSignature<TSignature>(
                    slimHandle);
            }
            ...
            auto isDead() const noexcept { return manager.isDead(slimHandle); }

            auto kill() noexcept { return manager.kill(slimHandle); }
            ...
            {
                return manager.template emplaceComponent<TComponent>(
                    slimHandle);
            }
