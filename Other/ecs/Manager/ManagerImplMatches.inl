...
namespace ecs
{
	namespace Impl
	{
		...
		struct Manager<TSettings>::MatchCallHelper
		{
			...
			static void call(EntityIndex mX, ThisType& mManager, TFunction&& mFunction)
			{
				mFunction
				(
					mX,
					mManager.template getComponent<TComponents>(mX)...
				);
			}
		};
		...
		void Manager<TSettings>::expandMatchCall(EntityIndex mX, TFunction&& mFunction) noexcept
		{
		...
		auto Manager<TSettings>::matchesSignatureImpl(const EntityBitset& mEntityBitset, NoForbiddenElements) const noexcept
		{
			const auto& requiresBitset(getRequiresBitset<TSignature>());

			// Does the entity have all required components?
			if((requiresBitset & mEntityBitset) != requiresBitset)
				return false;
		...
		template<typename TSettings>
		template<typename TSignature> 
		auto Manager<TSettings>::matchesSignatureImpl(const EntityBitset& mEntityBitset, HasForbiddenElements) const noexcept
		{
			if(!matchesSignatureImpl<TSignature>(mEntityBitset, std::false_type{}))
				return false;
		...
		auto Manager<TSettings>::matchesSignature(EntityIndex mX) const noexcept
		{
			...
			(
				getEntityBitset(mX), 
				DispatchForbiddenElements<TSignature>{}
			);
		}

		template<typename TSettings>
		template<typename TSignature>
		auto Manager<TSettings>::matchesSignature(const SlimHandle& mX) const noexcept
		{
		...
		template<typename TSettings>
		template<template<typename> class TSystem>
		auto Manager<TSettings>::matchesSystem(const SlimHandle& mX) const noexcept
		{
