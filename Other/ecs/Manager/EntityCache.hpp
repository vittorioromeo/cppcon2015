	...
	namespace Impl
	{
		...
		class Manager<TSettings>::EntityCache final
		{
			private:
				...
				using SignatureCache = typename SignatureConfig::template CacheTuple<CacheType>;
				...
				{
					Utils::forTuple([&mFunction](auto& mX)
					{
						mFunction(mX);
					}, signatureCache);
				}
				...
				{
					return std::get<getSignatureTypeID<TSignature>()>(signatureCache);
				}
