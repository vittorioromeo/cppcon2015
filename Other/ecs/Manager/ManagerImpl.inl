    ...
    namespace Impl
    {
        ...
        const auto& Manager<TSettings>::getRequiresBitset() const noexcept
        {
            return bitsetStorage
                .template getSignatureRequiresBitset<TSignature>();
        }
        ...
        {
            return bitsetStorage
                .template getSignatureForbidsBitset<TSignature>();
        }
        ...
        {
            return componentConfig.template getComponentByDataIndex<TComponent>(
                mX);
        }
        ...
        template <typename TSettings>
        const auto& Manager<TSettings>::getEntityBitset(EntityIndex mX) const
            noexcept
        {
        ...
        template <typename TSettings>
        const auto& Manager<TSettings>::getHandleData(HandleDataIndex mX) const
            noexcept
        {
            assert(capacity > mX);
            return handleData[mX];
        ...
        template <typename TSettings>
        const auto& Manager<TSettings>::getHandleData(EntityIndex mX) const
            noexcept
        {
            return getHandleData(getEntity(mX).handleDataIndex);
        }
        ...
        template <typename TSettings>
        const auto& Manager<TSettings>::getHandleData(
            const SlimHandle& mX) const noexcept
        {
            return getHandleData(mX.handleDataIndex);
        }
        ...
        template <typename TSettings>
        auto Manager<TSettings>::isHandleValid(const SlimHandle& mX) const
            noexcept
        {
        ...
        template <typename TSettings>
        auto Manager<TSettings>::getEntityIndex(const SlimHandle& mX) const
            noexcept
        {
        ...
        struct Manager<TSettings>::ComponentFinder<TCurrent, TRest...>
        {
            template <typename TFunction>
            static void call(EntityIndex mX,
                Manager<TSettings>::ThisType& mManager, TFunction&& mFunction)
            {
        ...
        template <typename TSettings>
        template <typename TFunction>
        void Manager<TSettings>::forComponents(
            TFunction&& mFunction, EntityIndex mX) noexcept
        {
            assert(isAlive(mX));
            MPL::Rename<ComponentFinder,
                typename ComponentConfig::ComponentTypes>::call(mX, *this,
                mFunction);
        }
        ...
        void Manager<TSettings>::forEntitiesMatchingSignature(
            TFunction&& mFunction) noexcept
        {
            ...
            {
                entityCache.template forCache<TSignature>(
                    [this, &mFunction](auto mI)
                    {
        ...
        void Manager<TSettings>::forEntitiesMatching(
            TFunction&& mFunction) noexcept
        {
        ...
        void Manager<TSettings>::forSystem0(TFunction&& mFunction)
        {
            ...
            forEntitiesMatching<TSystem>(
                [this, &system, &mFunction](auto&&... xs)
                {
        ...
        template <template <typename> class TSystem, typename... Ts,
            typename... TArgs>
        void Manager<TSettings>::forSystem(
            void (TSystem<ThisType>::*mFunction)(Ts...), TArgs&&... mArgs)
        {
            auto& system(this->getSystem<TSystem>());
            forEntitiesMatching<TSystem>(
                [this, &system, &mFunction, &mArgs...](auto&&... xs)
                {
