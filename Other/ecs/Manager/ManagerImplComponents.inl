...
namespace ecs
{
	namespace Impl
	{
		...
		template<typename TSettings>
		template<typename TComponent>
		auto Manager<TSettings>::hasComponent(const SlimHandle& mX) const noexcept
		{
		...
		auto& Manager<TSettings>::emplaceComponent(EntityIndex mX, Ts&&... mXs) noexcept
		{
			assert(isAlive(mX));
			assert(!hasComponent<TComponent>(mX));
		...
		template<typename TSettings>
		template<typename TComponent, typename... Ts>
		auto& Manager<TSettings>::emplaceComponent(const SlimHandle& mX, Ts&&... mXs) noexcept
		{
			return emplaceComponent<TComponent>(getEntityIndex(mX), ECS_FWD(mXs)...);
		}
