	...
	namespace Impl
	{
		...
		class Manager final
		{
			...
			private:
				...
				using SystemListWithManager = typename SystemConfig::template AsTypeListWithManager<ThisType>;
				static_assert(MPL::Rename<Traits::AreSystem, SystemListWithManager>{}, "");
				...
				template<typename... TComponents> struct MatchCallHelper;
				template<typename... TComponents> struct ComponentFinder;
				...
				// Entity metadata storage. Guarantees contiguous alive entities after refresh.
				...
				// Next size (after refresh). Takes into account newly created entities.
				...
				template<typename TSignature> const auto& getRequiresBitset() const noexcept;
				template<typename TSignature> const auto& getForbidsBitset() const noexcept;

				// Get a component from data index.
				template<typename TComponent> auto& getComponentByDataIndex(DataIndex mX) noexcept;
				...
				void expandMatchCall(EntityIndex mX, TFunction&& mFunction) noexcept;

				// Refresh the manager and return "one past last alive entity" index.
			...
			public:				
				...
				// Matches signature?
				template<typename TSignature> auto matchesSignatureImpl(const EntityBitset& mEntityBitset, NoForbiddenElements) const noexcept;
				template<typename TSignature> auto matchesSignatureImpl(const EntityBitset& mEntityBitset, HasForbiddenElements) const noexcept;
				template<typename TSignature> auto matchesSignature(EntityIndex mX) const noexcept;
				template<typename TSignature> auto matchesSignature(const SlimHandle& mX) const noexcept;

				// Matches system?
				template<template<typename> class TSystem> auto matchesSystem(EntityIndex mX) const noexcept;
				template<template<typename> class TSystem> auto matchesSystem(const SlimHandle& mX) const noexcept;
				...
				// Has tag?
				template<typename TTag> auto hasTag(EntityIndex mX) const noexcept;
				template<typename TTag> auto hasTag(const SlimHandle& mX) const noexcept;

				// Adds a tag.
				template<typename TTag> void addTag(EntityIndex mX) noexcept;
				template<typename TTag> void addTag(const SlimHandle& mX) noexcept;

				// Removes a tag.
				template<typename TTag> void removeTag(EntityIndex mX) noexcept;
				template<typename TTag> void removeTag(const SlimHandle& mX) noexcept;

				// Has component?
				template<typename TComponent> auto hasComponent(EntityIndex mX) const noexcept;
				template<typename TComponent> auto hasComponent(const SlimHandle& mX) const noexcept;

				// Adds a component.
				template<typename TComponent, typename... Ts> auto& emplaceComponent(EntityIndex mX, Ts&&... mXs) noexcept;
				template<typename TComponent, typename... Ts> auto& emplaceComponent(const SlimHandle& mX, Ts&&... mXs) noexcept;

				// Removes a component.
				template<typename TComponent> void removeComponent(EntityIndex mX) noexcept;
				template<typename TComponent> void removeComponent(const SlimHandle& mX) noexcept;

				// Gets a reference to a component.
				template<typename TComponent> auto& getComponent(EntityIndex mX) noexcept;
				template<typename TComponent> auto& getComponent(const SlimHandle& mX) noexcept;

				// Iterates over an entity's components.
				template<typename TFunction> void forComponents(TFunction&& mFunction, EntityIndex mX) noexcept;
				template<typename TFunction> void forComponents(TFunction&& mFunction, const SlimHandle& mX) noexcept;
				...
				void forSystem(void(TSystem<ThisType>::*mFunction)(Ts...), TArgs&&... mArgs);
				...
				void touchAliveEntity(EntityIndex mX)
				{
					...
					MPL::forTypes<typename SignatureConfig::AsTypeList>([this, mX](auto mSignature)
					{
