...
namespace ecs
{
    namespace Impl
    {
        ...
        private:
            ...
            // Pairs every signature to a specific bitset type, inside of a
            // tuple.
            template <typename TBitset>
            using BitsetPairTuple = std::tuple<std::pair<TSignatures,
                typename TSignatures::template BitsetStorage<TBitset>>...>;

            // TODO:
            template <typename TCache>
            using CacheTuple = MPL::Rename<std::tuple,
                MPL::Repeat<MPL::size<AsTypeList>(), TCache>>;
        ...
        template <typename TSignature>
        using DispatchForbiddenElements =
            std::integral_constant<bool, TSignature::hasForbiddenElements>;
