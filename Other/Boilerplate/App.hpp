...
namespace Boilerplate
{
	...
	class TestApp : public Boilerplate::App
	{
		...
		private:
			...
			void initInput()
			{
				gameState.addInput({{IK::Escape}}, [this](FT){ stop(); }, IT::Once);
				gameState.addInput({{IK::A}}, [this](FT){ gameCamera.pan(-4, 0); });
				gameState.addInput({{IK::D}}, [this](FT){ gameCamera.pan(4, 0); });
				gameState.addInput({{IK::W}}, [this](FT){ gameCamera.pan(0, -4); });
				gameState.addInput({{IK::S}}, [this](FT){ gameCamera.pan(0, 4); });
				gameState.addInput({{IK::Q}}, [this](FT){ gameCamera.zoomOut(1.1f); });
				gameState.addInput({{IK::E}}, [this](FT){ gameCamera.zoomIn(1.1f); });
			}
			...
			void updateTxtInfo(FT mFT)
			{
				...
				ossInfo	<< "FPS: " << gameWindow.getFPS() << "\n"
						<< "U: " << gameWindow.getMsUpdate() << "\t" << "D: " << gameWindow.getMsDraw() << "\n";
		...
		public:
			...
			void enableTxtInfo(FT mUpdateRate)
			{
				...
				gameState.onUpdate += [this](FT mFT){ updateTxtInfo(mFT); };
				gameState.onDraw += [this]{ drawTxtInfo(); };
			}
